use riffl_core::format::{it::import_it, protracker::import_mod, xm::import_xm};
...
fn import_xm_file(path: &std::path::Path) -> Result<riffl_core::format::FormatData, String> {
    let data = std::fs::read(path).map_err(|e| e.to_string())?;
    import_xm(&data).map_err(|e| format!("{:?}", e))
}

fn import_mod_file(path: &std::path::Path) -> Result<riffl_core::format::FormatData, String> {
    let data = std::fs::read(path).map_err(|e| e.to_string())?;
    import_mod(&data).map_err(|e| format!("{:?}", e))
}

fn import_it_file(path: &std::path::Path) -> Result<riffl_core::format::FormatData, String> {
    let data = std::fs::read(path).map_err(|e| e.to_string())?;
