...
impl Envelope {
    /// Evaluates the envelope value at the given tick and fractional progress between ticks,
    /// handling sustain and loop points. Returns a tuple of `(value, next_tick)`.
    pub fn evaluate(&self, tick: usize, fraction: f32, key_on: bool) -> (f32, usize) {
        ...
        if self.sustain_enabled && key_on {
            let sus_end = self
                ...
                .map(|p| p.frame)
                .unwrap_or(0);
            if tick >= sus_end as usize {
                loop_start_frame = self
                    .points
                    .get(self.sustain_start_point)
                    .map(|p| p.frame)
                    .unwrap_or(0);
                next_tick = loop_start_frame as usize;
                loop_wrap = true;
            }
        ...
        // Current evaluation position including fraction
        let eval_pos = tick as f32 + fraction;

        // If we are wrapping around a loop, we interpolate from current point back to start
        if loop_wrap {
            // Find current point (which is the end of loop)
            if let Some(p_end) = self.points.iter().find(|p| p.frame as usize == tick) {
        ...
        // Clamp to end if no loop/sustain or if we've passed the last point
        let last_point = self.points.last().unwrap();
        if eval_pos >= last_point.frame as f32 {
            return (last_point.value, next_tick.min(tick));
        }

        // Normal segment lookup
        for i in 0..self.points.len() - 1 {
            let p1 = &self.points[i];
            let p2 = &self.points[i + 1];
            if eval_pos >= p1.frame as f32 && eval_pos < p2.frame as f32 {
                let range = (p2.frame - p1.frame) as f32;
                let segment_fraction = (eval_pos - p1.frame as f32) / range;
                let value = p1.value + (p2.value - p1.value) * segment_fraction;
                return (value, next_tick);
            }
        }

        (last_point.value, next_tick)
    }
}
