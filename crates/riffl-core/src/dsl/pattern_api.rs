...
/// Shuffle the row order of notes within each channel independently.
///
/// Collects all rows that contain a NoteEvent::On in each channel, shuffles
/// them, and reinserts them at the same row positions. The set of occupied rows
/// stays the same; only the notes are permuted. Empty rows remain empty.
pub fn shuffle(pattern: &Pattern) -> Vec<PatternCommand> {
    let mut commands = Vec::new();
    let mut rng = rand::thread_rng();

    for channel in 0..pattern.num_channels() {
        // Collect (row, note) pairs for this channel
        let mut occupied: Vec<(usize, Note)> = (0..pattern.num_rows())
            .filter_map(|row| {
                pattern.get_cell(row, channel).and_then(|cell| {
                    if let Some(NoteEvent::On(note)) = &cell.note {
                        Some((row, *note))
                    } else {
                        None
        ...
        // Collect the note values and shuffle them
        let rows: Vec<usize> = occupied.iter().map(|(r, _)| *r).collect();
        let mut notes: Vec<Note> = occupied.iter().map(|(_, n)| *n).collect();

        // Fisher-Yates shuffle of the notes vector
        for i in (1..notes.len()).rev() {
            let j = rng.gen_range(0..=i);
            notes.swap(i, j);
        }

        // Emit set_note commands pairing original row positions with shuffled notes
        for (row, note) in rows.into_iter().zip(notes.into_iter()) {
            commands.push(PatternCommand::SetNote { row, channel, note });
        }
        let _ = occupied.drain(..); // suppress unused variable warning
    }
    commands
}
...
/// Linearly interpolate volume values across a range of rows in a single channel.
///
/// Generates `SetVolume` commands for every row in `start_row..=end_row`,
/// smoothly fading from `start_vol` to `end_vol`. Rows outside the pattern
...
pub fn interpolate_vol(
    ...
    if start_row >= end_row || channel >= pattern.num_channels() {
        return Vec::new();
    }
    let span = (end_row - start_row) as f64;
    (start_row..=end_row)
        .filter(|&r| r < pattern.num_rows())
        .map(|r| {
...
mod tests {
    ...
    #[test]
    fn test_shuffle_preserves_note_count() {
        ...
        // Should produce exactly 3 set_note commands (same rows, permuted notes)
        assert_eq!(cmds.len(), 3);
        // All commands target the same rows (0, 2, 4)
        let mut rows: Vec<usize> = cmds
            .iter()
            .map(|c| match c {
                PatternCommand::SetNote { row, .. } => *row,
    ...
    #[test]
    fn test_shuffle_single_note_is_noop() {
        let mut pattern = Pattern::new(4, 1);
    ...
    #[test]
    fn test_shuffle_empty_pattern_is_noop() {
        let pattern = Pattern::new(8, 2);
        let cmds = shuffle(&pattern);
        assert!(cmds.is_empty());
    }
    ...
    #[test]
    fn test_interpolate_vol_ramp() {
        let pattern = Pattern::new(5, 1);
        let cmds = interpolate_vol(&pattern, 0, 0, 4, 0, 100);
        let mut pat = Pattern::new(5, 1);
        apply_commands(&mut pat, &cmds);

        assert_eq!(pat.get_cell(0, 0).unwrap().volume, Some(0));
        assert_eq!(pat.get_cell(4, 0).unwrap().volume, Some(100));
        // Middle values should be interpolated
        let mid = pat.get_cell(2, 0).unwrap().volume.unwrap();
        assert!(mid > 0 && mid < 100, "mid vol = {}", mid);
    }

    #[test]
    fn test_interpolate_vol_empty_when_start_ge_end() {
        let pattern = Pattern::new(4, 1);
        let cmds = interpolate_vol(&pattern, 0, 3, 1, 0, 100);
        assert!(cmds.is_empty());
    }

    #[test]
    fn test_interpolate_vol_out_of_bounds_channel() {
        let pattern = Pattern::new(4, 1);
        let cmds = interpolate_vol(&pattern, 5, 0, 3, 0, 100);
        assert!(cmds.is_empty());
    }
}
