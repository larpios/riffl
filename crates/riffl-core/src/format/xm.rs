...
fn convert_xm_volume_column(vol: u8) -> (Option<u8>, Option<Effect>) {
    match vol >> 4 {
        ...
        0xF => {
            // Tone portamento: param nibble is the direct speed (0–15), not packed.
            let param = vol & 0x0F;
            (None, Some(Effect::new(0x03, param)))
        }
        _ => (None, None),
    }
}
...
pub fn import_xm(data: &[u8]) -> Result<FormatData, String> {
    ...
    for xm_inst in &xm_instruments {
        ...
        for (s_idx, sh) in xm_inst.samples.iter().enumerate() {
            ...
            // XM has no instrument-level global volume — sample volume (set above) carries it.
            // inst.volume stays at 1.0 (default) to avoid double-counting with sample.volume.
