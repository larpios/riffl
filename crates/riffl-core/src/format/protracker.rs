...
///
/// Scans patterns in arrangement order so the initial BPM/speed reflects what
/// the song actually starts with, not the lowest-indexed pattern in the pool.
/// Returns (speed, bpm) indicating the first Speed (TPL) and BPM found, defaulting to 6 and 125.0.
fn extract_tempo_from_patterns(patterns: &[Pattern], arrangement: &[usize]) -> (u8, f64) {
...
pub fn import_mod(data: &[u8]) -> Result<super::FormatData, String> {
    ...
    let (speed, bpm) = extract_tempo_from_patterns(&patterns, &arrangement);
...
mod tests {
    ...
    fn test_extract_tempo_pattern_with_fxx_effect() {
        ...
        let (speed, bpm) = extract_tempo_from_patterns(&patterns, &[0]);
        assert_eq!(speed, 5);
        assert_eq!(bpm, 125.0);
    }

    #[test]
    fn test_extract_tempo_respects_arrangement_order() {
        // Pattern 0: has F=150 (BPM 150)
        ...
        assert_eq!(bpm, 180.0); // Should pick up pattern 1's BPM, not pattern 0's
        assert_eq!(speed, 6);
    }
