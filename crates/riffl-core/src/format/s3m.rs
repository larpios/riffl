...
pub fn import_s3m(data: &[u8]) -> Result<FormatData, String> {
    ...
    for (i, &ptr) in header.inst_pointers.iter().enumerate() {
        ...
        if let Some(float_data) = s3m_inst.sample_data {
            ...
            // sample.sample_rate is already set to c2spd (from the struct field above).
                ...
                #[cfg(feature = "adlib")]
                {
                    ...
                    match adlib_result {
                        Ok(data) => {
                            ...
                            sample.volume = s3m_inst.volume as f32 / 64.0;
                            // Adlib sample rendered at SAMPLE_RATE; set base_note=48 (C-4)
                            // so the engine plays it at the correct pitch.
                            sample = sample.with_base_note(48);
                            sample
                        }
                        Err(_) => {
