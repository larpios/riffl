...
impl ItSampleHeader {
    ...
    fn is_sustain_pingpong(&self) -> bool {
        self.use_sustain_loop() && self.flags & 0x80 != 0
    }
...
pub fn import_it(data: &[u8]) -> Result<FormatData, String> {
    ...
    for (s_idx, sh) in sample_headers.iter().enumerate() {
        if let Some(ref float_data) = sample_data_vec[s_idx] {
            if float_data.is_empty() {
                // Keep sample list aligned with header offsets
                out_samples.push(Sample::default());
                continue;
            }
            ...
            if sh.use_sustain_loop() && sh.sustain_loop_end > sh.sustain_loop_start {
                ...
                if sus_end > sus_start {
                    let mode = if sh.is_sustain_pingpong() {
                        LoopMode::PingPong
                    } else {
                        LoopMode::Forward
                    };
    ...
    if header.use_instruments {
        // Create one Riffl Instrument for each IT Instrument
        for it_inst in &it_instruments {
            ...
            inst.volume_envelope = it_inst.volume_envelope.clone();
            ...
            for (note_idx, &(target_note, sample_num)) in it_inst.note_sample_table.iter().enumerate() {
                if sample_num > 0 {
                    ...
                    if s_idx < out_samples.len() {
                        let mut kz = Keyzone::new(s_idx);
                        kz.note_min = note_idx as u8;
                        kz.note_max = note_idx as u8;
                        kz.base_note_override = Some(target_note);
        ...
        // Direct Sample Mode: create one Instrument per Sample
        for (s_idx, sh) in sample_headers.iter().enumerate() {
            let mut inst = Instrument::new(if sh.name.is_empty() {
                sh.dos_filename.clone()
            } else {
                sh.name.clone()
            });
            ...
            for c_idx in 0..num_channels {
                ...
                if let Some(NoteEvent::On(mut n)) = note_event {
                    ...
                    if let Some(i) = inst_idx {
                        ...
                        // In sample mode, we can play if we have a sample index
                        if !header.use_instruments {
                            if let Some(i) = inst_idx {
                                n.instrument = i as u8;
                                cell.note = Some(NoteEvent::On(n));
