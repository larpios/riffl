        ...
        for effect in effects {
            ...
            match effect_type {
                ...
                EffectType::PortamentoToNote => {
                    if effect.param > 0 {
                        let speed = match state.slide_mode {
                            // apply_portamento expects speed in semitones/tick for Linear mode.
                            // FT2 XM: portaSpeed * 4 linear units / 64 units-per-semitone = param/16.
                ...
                EffectType::VolumeSlide => {
                    if effect.param > 0 {
                        if effect.param_x() > 0 && effect.param_y() == 0x0F {
                ...
                EffectType::Extended => {
                    ...
                    match sub_command {
                        0x1 => match state.slide_mode {
                            SlideMode::Linear => {
                                // E1x fine porta up: 1 linear unit = 1/64 semitone (FT2 XM spec).
                                // This is 1/4 the resolution of 1xx (which moves 1/16 semitone per unit).
                                let semitones = sub_param as f64 / 64.0;
                        ...
                        0x2 => match state.slide_mode {
                            SlideMode::Linear => {
                                // E2x fine porta down: same scale as E1x, 1/64 semitone per unit.
                                let semitones = sub_param as f64 / 64.0;
                                state.pitch_ratio *= 2.0_f64.powf(-semitones / 12.0);
                            }
                            SlideMode::AmigaPeriod => {
                ...
                EffectType::ChannelVolumeSlide => {
                    if effect.param_x() > 0 && effect.param_y() == 0x0F {
                        // Fine channel volume slide up (xF): applied once at row start
                        state.channel_volume = (state.channel_volume
                            + (effect.param_x() as f32 / 64.0))
                            .clamp(0.0, 1.0);
                    } else if effect.param_x() == 0x0F && effect.param_y() > 0 {
                        // Fine channel volume slide down (Fy): applied once at row start
                        state.channel_volume = (state.channel_volume
...
mod tests {
    ...
    /// Portamento (3xx) Linear speed: param / 16 semitones/tick, matching 1xx/2xx parity.
    /// apply_portamento(Linear) uses 2^(speed/12), so speed=1 = 1 semitone/tick.
    /// FT2 XM: portaSpeed * 4 linear-freq-units / 64 units-per-semitone = param / 16.
    #[test]
    fn test_portamento_to_note_linear_speed_matches_slide_scale() {
        let mut proc = TrackerEffectProcessor::new(1, 48000);
        let effects = vec![Effect::from_type(EffectType::PortamentoToNote, 0x10)]; // param = 16
        proc.process_row(0, &effects, Some(880.0));
        let state = proc.channel_state(0).unwrap();
        // param=16 / 16 = 1.0 semitone per tick (same perceived speed as 1xx param=16)
        assert!(
            (state.portamento_speed - 1.0).abs() < 0.001,
            "Expected 1.0, got {}",
            state.portamento_speed
        );
    }
}
