                ...
                for frame in 0..num_frames {
                    ...
                    if !triggered_now {
                        if let Some(retrigger_interval) = ch_state.retrigger_interval {
                            ...
                            {
                                voice.position = 0.0;
                                // Reset envelope frame counters on retrigger
                                voice.volume_envelope_frame = 0.0;
                                voice.panning_envelope_frame = 0.0;
                                voice.pitch_envelope_frame = 0.0;
                    ...
                    if let Some(inst) = self.instruments.get(voice.instrument_index) {
                        // Update LFO phases
                        voice.lfo.update(inst, self.bpm, output_sample_rate);

                        // Fadeout processing (applied once per voice-relative envelope tick)
                        if !voice.key_on {
                            if inst.fadeout > 0 {
                            ...
                            } else if inst.volume_envelope.is_none() && inst.volume_adsr.is_none() {
                                // No fadeout and no volume envelopes: cut immediately on Note Off (Sample Mode behavior)
                                voice.active = false;
                            }
                        }
                        ...
                        if let Some(vol_env) = &inst.volume_envelope {
                            if vol_env.enabled {
                                ...
                                if voice.volume_envelope_frame >= frames_per_tick_f32 {
                                    voice.volume_envelope_frame -= frames_per_tick_f32;
                        ...
                        if let Some(pan_env) = &inst.panning_envelope {
                            if pan_env.enabled {
                                ...
                                if voice.panning_envelope_frame >= frames_per_tick_f32 {
                                    voice.panning_envelope_frame -= frames_per_tick_f32;
                        ...
                        if let Some(pitch_env) = &inst.pitch_envelope {
                            if pitch_env.enabled {
                                ...
                                if voice.pitch_envelope_frame >= frames_per_tick_f32 {
                                    voice.pitch_envelope_frame -= frames_per_tick_f32;
