...
pub struct Pattern {
    ...
    tracks: Vec<Track>,
    /// Optional user-defined name for this pattern.
    #[serde(default)]
    pub name: String,
}
...
impl Pattern {
    ...
    /// Set a full row at the given position.
    ///
    /// Returns true if the position was valid and the row was set,
    /// false if out of bounds.
    pub fn set_row(&mut self, row_idx: usize, row_data: Vec<Cell>) -> bool {
        if row_idx < self.rows.len() && row_data.len() == self.channels {
