...
impl ChannelEffectState {
    ...
    pub fn vibrato_pitch_ratio(&self) -> f64 {
        ...
        let modulation = match self.vibrato_waveform {
            0 => self.vibrato_phase.sin(),
            1 => 1.0 - (self.vibrato_phase / std::f64::consts::PI),
            2 => {
            ...
            _ => (self.vibrato_phase * 1000.0).sin().fract() * 2.0 - 1.0,
        } * depth_semitones;
    ...
    pub fn tremolo_amplitude_modulation(&self) -> f32 {
        ...
        let modulation = match self.tremolo_waveform {
            0 => self.tremolo_phase.sin(),
            1 => 1.0 - (self.tremolo_phase / std::f64::consts::PI),
            2 => {
            ...
            _ => (self.tremolo_phase * 1000.0).sin().fract() * 2.0 - 1.0,
        };
