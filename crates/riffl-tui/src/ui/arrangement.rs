...
impl ArrangementView {
    ...
    pub fn create_new_pattern(&mut self, song: &mut Song) -> Option<usize> {
        ...
        let mut new_pattern = riffl_core::pattern::Pattern::new(64, num_channels);
        new_pattern.name = format!("Pattern {:02}", song.patterns.len() + 1);
    ...
    /// Swap the entry at the cursor with the one above it (move entry up).
    /// Returns true if the swap was performed.
    pub fn move_entry_up(&mut self, song: &mut Song) -> bool {
        if self.cursor == 0 || song.arrangement.len() < 2 {
            return false;
        }
        song.arrangement.swap(self.cursor - 1, self.cursor);
        self.cursor -= 1;
        true
    }

    /// Swap the entry at the cursor with the one below it (move entry down).
    /// Returns true if the swap was performed.
    pub fn move_entry_down(&mut self, song: &mut Song) -> bool {
        let len = song.arrangement.len();
        if len < 2 || self.cursor >= len - 1 {
            return false;
        }
        song.arrangement.swap(self.cursor, self.cursor + 1);
        self.cursor += 1;
        true
    }
...
fn pattern_preview(pattern: &riffl_core::pattern::Pattern) -> String {
...
mod tests {
    ...
    #[test]
    fn test_create_new_pattern_has_auto_name() {
        let mut song = test_song();
        let before = song.patterns.len();
        let mut view = ArrangementView::new();
        let idx = view.create_new_pattern(&mut song).unwrap();
        let expected = format!("Pattern {:02}", before + 1);
        assert_eq!(song.patterns[idx].name, expected);
    }
    ...
    #[test]
    fn test_move_entry_up_swaps_and_moves_cursor() {
        let mut song = test_song(); // arrangement: [0, 1, 0]
        let mut view = ArrangementView::new();
        view.cursor = 1;

        assert!(view.move_entry_up(&mut song));
        assert_eq!(song.arrangement, vec![1, 0, 0]);
        assert_eq!(view.cursor(), 0);
    }

    #[test]
    fn test_move_entry_up_at_top_does_nothing() {
        let mut song = test_song();
        let mut view = ArrangementView::new();
        view.cursor = 0;

        assert!(!view.move_entry_up(&mut song));
        assert_eq!(song.arrangement, vec![0, 1, 0]);
        assert_eq!(view.cursor(), 0);
    }

    #[test]
    fn test_move_entry_down_swaps_and_moves_cursor() {
        let mut song = test_song(); // arrangement: [0, 1, 0]
        let mut view = ArrangementView::new();
        view.cursor = 1;

        assert!(view.move_entry_down(&mut song));
        assert_eq!(song.arrangement, vec![0, 0, 1]);
        assert_eq!(view.cursor(), 2);
    }

    #[test]
    fn test_move_entry_down_at_bottom_does_nothing() {
        let mut song = test_song();
        let mut view = ArrangementView::new();
        view.cursor = 2; // last entry
    ...
    // --- Clone/duplicate pattern via arrangement ---

    #[test]
    fn test_create_new_pattern_inserts_at_end_of_arrangement() {
        let mut song = test_song();
        // test_song: arrangement = [0, 1, 0], 2 patterns
        let mut view = ArrangementView::new();
        view.cursor = 2; // last position

        let idx = view.create_new_pattern(&mut song);
        assert!(idx.is_some());
        // New pattern appended after cursor (position 3)
        assert_eq!(view.cursor(), 3);
        assert_eq!(song.arrangement.len(), 4);
        assert_eq!(*song.arrangement.last().unwrap(), idx.unwrap());
    }
}
