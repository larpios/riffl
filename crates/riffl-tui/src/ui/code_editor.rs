...
mod tests {
    ...
    fn test_template_up_down_navigation() {
        use riffl_core::dsl::examples::TEMPLATES;
        let last = TEMPLATES.len() - 1;

        let mut ed = CodeEditor::new();
        ed.toggle_templates();

        // Step down to the last entry
        for expected in 1..=last {
            ed.template_down();
            assert_eq!(ed.template_cursor, expected);
        }
        // Can't go past last
        ed.template_down();
        assert_eq!(ed.template_cursor, last);

        // Step back up to the first entry
