...
impl App {
    ...
    pub fn adjust_track_volume(&mut self, delta: f32) {
        let ch = self.editor.cursor_channel();
        if let Some(track) = self.editor.pattern_mut().get_track_mut(ch) {
            track.set_volume((track.volume + delta).clamp(0.0, 1.0));
        }
        if let Some(track) = self.song.tracks.get_mut(ch) {
            track.set_volume((track.volume + delta).clamp(0.0, 1.0));
        }
    ...
    /// Adjust panning of the current track by `delta` (range -1.0..=1.0, step ~0.1).
    /// Syncs with the global song track state and the mixer.
    pub fn adjust_track_pan(&mut self, delta: f32) {
        let ch = self.editor.cursor_channel();
        if let Some(track) = self.editor.pattern_mut().get_track_mut(ch) {
            track.set_pan((track.pan + delta).clamp(-1.0, 1.0));
        }
        if let Some(track) = self.song.tracks.get_mut(ch) {
            track.set_pan((track.pan + delta).clamp(-1.0, 1.0));
        }
