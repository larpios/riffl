...
pub(super) fn handle_action(app: &mut App, action: Action, key: KeyEvent) {
    ...
    match action {
        ...
        // Go to specific row — enter command mode pre-filled with "goto "
        Action::GoToRow => {
        ...
        // Transport
        Action::TogglePlay => {
            // In instrument list, Space previews the selected instrument instead
            if app.current_view == AppView::InstrumentList {
        ...
        Action::Confirm => {
            if app.pending_quit {
                ...
                // Play From Cursor: Enter while stopped or paused starts playback at the cursor row
